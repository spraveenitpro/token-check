...
token-check openai "Hello, world!"
```
...
echo "Hello, world!" | uv run token-check gemini -

# Specify a model
uv run token-check openai "Hello, world!" --model gpt-4-turbo
uv run token-check gemini "Hello, world!" --model gemini-1.5-pro

# Quiet mode (output only the number)
uv run token-check openai "Hello, world!" --quiet
# Output: 4
...
uv run token-check anthropic "Hello, world!" --cost --quiet
# Output: 12,0.000036
